...
Required evidence: successful configure/build/install, actual installed executable and runtime resources, ZIP integrity and independent SHA-256, fresh Windows runner without development Qt, bounded version and score export, TC11/TC14 GUI assertions, same-source Linux unit success, and opt-in draft prerelease with identical tested bytes. The cold hosted run is qualified; exact whole-run warm cache qualification remains pending, and legacy duplicate builds remain until replacement parity is demonstrated.

The current trusted-cache contract is narrow: push and manual runs default to cache enabled, while pull requests use an effective cache value of `false`. Compiler objects use the helper-owned `build-output/ccache` directory with a 4G bound and normal-profile `MUSE_COMPILE_USE_PCH=ON`; dependency restore/save handles only completed hash-named archives under `build.release/_deps/<payload>/.pinned/<64 lowercase SHA-256>`. Pull requests never restore or save these caches, and no build tree, `CMakeCache.txt`, extracted dependency tree, partial archive or identity metadata is restored. A cache-enabled warm run still needs the full build/package/runtime/readiness proof and real compiler hits.
...
2. **Cold hosted qualification achieved, later gates open.** Run `34777803748` completed green cold (`preflight`, `windows`, `units`, `runtime`, `readiness`) with the artifact identity and per-case runtime evidence recorded above, and the Linux units gate passed at the same source. Still unproven: any exact whole-run warm/`use_cache=true` qualification with real compiler hits, the opt-in draft-release path, and macOS/Linux release packaging.
