...
class TC15ScriptContractTests(unittest.TestCase):
    ...
    def test_readiness_wait_follows_bounded_observed_keyboard_submission(self):
        ...
        self.assertIn('var NEW_SCORE_SELECT_PATH = "NewScoreDialog/SelectPanel/Select"', self.source)
        self.assertIn('var NEW_SCORE_SCORE_LIST_PANEL = "ListView"', self.source)
        self.assertIn('var NEW_SCORE_BOTTOM_PANEL = "BottomPanel"', self.source)
        ...
        self.assertNotIn('api.keyboard.key("End")', submitter)
        self.assertIn('api.keyboard.key("Return")', submitter)
        self.assertIn("visitedPaths", submitter)
        self.assertIn("selectActivated", submitter)
        self.assertIn("afterSelect", submitter)
        self.assertIn("api.navigation.controls(", submitter)
        self.assertIn("Done control is not enabled after instrument selection", submitter)
        self.assertIn("Enabled Done control could not be triggered", submitter)
        self.assertNotIn("current.panel === NEW_SCORE_BOTTOM_PANEL", submitter)
        self.assertIn("indexOf", submitter)
        self.assertIn("Select made no navigation progress", submitter)
        self.assertIn("Select did not expose a populated score list or Done control", submitter)
        self.assertIn("Navigation cycle detected", submitter)
        self.assertIn("NEW_SCORE_NAV_MAX_TABS", submitter)
        self.assertEqual(self.source.count("api.navigation.triggerControl("), 1)
